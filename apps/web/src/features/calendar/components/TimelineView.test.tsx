...
describe('TimelineView move affordances and live feedback', () => {
  ...
  describe('Phase 3.2 — Origin Ghost Indicator', () => {
    it('renders origin ghost with timelineEventGhost class and clean non-interactive silhouette', () => {
      const event = makeEvent({
        allDay: false,
        title: 'Original Slot Event',
        startAt: '2026-09-15T14:00:00.000Z',
        endAt: '2026-09-15T15:00:00.000Z',
      });
      const occurrence: EventOccurrence = {
      ...
      expect(html).not.toContain('Original Slot Event');
      expect(html).not.toContain('<button');
    });

    it('positions origin ghost according to original minutes and layout', () => {
      const event = makeEvent({
        allDay: false,
      ...
      const html = renderToStaticMarkup(
        <OriginGhost
          occurrence={occurrence}
          originalMinutes={{ startMinute: 600, endMinute: 660 }}
          originalLayout={{ left: 0.5, width: 0.5 }}
          hourHeight={64}
          isWeek={false}
        />,
      );
      ...
      expect(html).toContain('left:calc(50% + 2px)');
      expect(html).toContain('width:calc(50% - 4px)');
      expect(html).toContain('--event-color:#6E8BFF');
    });

    it('does not render origin ghost in normal state without an active move drag', () => {
      const event = makeEvent({
        allDay: false,
        title: 'Static Event',
        startAt: '2026-09-15T14:00:00.000Z',
    ...
    it('renders origin ghost with timelineEventGhostExiting class and data-exiting attribute when isExiting is true', () => {
      const event = makeEvent({
        allDay: false,
        title: 'Exiting Ghost Event',
        startAt: '2026-09-15T14:00:00.000Z',
        endAt: '2026-09-15T15:00:00.000Z',
      });
      const occurrence: EventOccurrence = {
        key: 'exiting-ghost-key',
        occurrenceIndex: 0,
        start: Date.parse(event.startAt),
        end: Date.parse(event.endAt),
        event,
