...
2. **Intent Interpretation & Fail Closed**: Luna (`gpt-5.6-luna`, reasoning effort `low`; selected for both intent and ranking by the 2026-09-24 live Low vs Medium evaluation) extracts structured intent adhering strictly to `schedulingIntentSchema` (`packages/schemas/src/intent.schema.ts`). Validation is strictly fail-closed: malformed variants (missing weekday, missing or invalid hour, inverted duration ranges) throw `AI_INVALID_OUTPUT` instead of silently degrading to unconstrained constraints. An hour-only time ("after 4") with a null minute means on the hour (minute 0). A well-formed but nonexistent calendar date (e.g., `2026-02-30`) becomes a clarification question rather than an error, as do past dates.
...
6. **Deterministic Normalization (`@cal/domain/scheduling`)**:
   ...
   - `resolveSemanticTimeWindow` (`semantic-time.ts`): Named parts of the day are hard windows, applied before ranking so the ranker never sees (or rationalizes) a 2 PM "dinner". Luna only names an `occasion` (`breakfast` 7:00–10:30, `brunch` 10:00–14:00, `lunch` 11:30–14:00, `dinner` 17:00–21:30, `drinks` 16:00–23:00) or a `time_of_day` (`morning` 5:00–12:00, `afternoon` 12:00–17:00, `evening` 17:00–22:00); every window lives in that one file. Windows intersect scheduling hours and never widen them: unlike an explicit clock time, a semantic window does not open a named day to the personal band. Explicit clock times (`exact`, `after`, `before`, `between`) take precedence over the occasion ("dinner at 3 PM" means 3 PM, under the existing out-of-hours rules); `around` keeps the occasion window only when the time sits inside it. A window with no overlap with scheduling hours, or an occasion that contradicts the named time of day, returns `clarification_required` instead of widening.
