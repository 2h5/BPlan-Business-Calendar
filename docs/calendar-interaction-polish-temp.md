...
3. **What is currently next?** Phase 4 Keyboard Manipulation (or Phase 2.1 hardening / Phase 5 spatial transitions).
4. **What remains beyond Phase 3.2?** Keyboard manipulation (Phase 4), spatial view transitions (Phase 5), and regression hardening (Phase 2.1).
5. **What order should the remaining work happen in?** Core direct manipulation & feedback first, followed by keyboard shortcuts and spatial continuity transitions.
...
## Phase 3.2 — Origin Ghost Indicator [COMPLETE]

Implemented a subtle, non-interactive origin ghost silhouette at the event's original date/time position while whole-event move dragging is active in Day and Week views.

### Capabilities:

- **Gesture-Only Activation**:
  - Ghost outline activates only after the drag gesture actually begins (exceeding the $\ge 6\text{px}$ movement threshold).
  - Ordinary clicks and pending click states ($< 6\text{px}$) never reveal or flash the ghost.
- **Original Geometry & Wall-Clock Fidelity**:
  - Uses the event's original start/end minute, original dateKey, and initial layout columns (`placed.left`, `placed.width`).
  - Ghost remains strictly stationary at its original slot throughout the drag gesture, regardless of where the live moving event is dragged.
- **Same-Day and Cross-Day Week Dragging**:
  - Same-day move: ghost remains pinned at original time while live event moves vertically.
  - Cross-day move in Week view: ghost remains pinned in the original day column at original time while live event moves across adjacent day columns.
  - Returning to original slot: live event hovers over the ghost with higher `z-index: 5` (`ghost` is `z-index: 1`).
- **Clean Visual Treatment**:
  - Restrained dashed outline (`1px dashed` perimeter with `3px dashed` left border matching event card structure) and subtle 6% transparent background tint based on `--event-color`.
  - Non-interactive: `pointer-events: none` ensures zero interference with drag tracking, hover states, or column drops.
  - Pure silhouette: `aria-hidden="true"`, with no distracting title or button clones.
- **Immediate Cleanup**:
  - Ghost unmounts immediately on `pointerup`, `pointercancel`, Escape key, window drag finish, or gesture reset.
  - Does not linger into or fight the release settle animation (Phase 3.3).
- **Strict Gesture Isolation**:
  - Applies solely to whole-event movement (`onMoveEvent` / `isDraggingMove`). Never renders during edge resizing.
- **Persistence & Coordination Invariants Preserved**:
  - Zero side effects on provider writes, database mutations, magnetic snapping, conflict checks, auto-scrolling, Undo, or timing math.
- **Chromium Blur Safety & Reduced-Motion Polish**:
  - Transient 120ms fade-in keyframe (`timeline-ghost-fade-in`) settles cleanly to `opacity: 1` with no transform, filters, or persistent `will-change`.
  - Fixed reduced-motion gap: `.timelineEventMoving`, `.timelineEventResizing`, and `.timelineEventGhost` are all included in `@media (prefers-reduced-motion: reduce)` with `transition: none !important; animation: none !important;`.
...
## Phase 4 — Keyboard Manipulation [SKIPPED / deferred by design]

Keyboard nudging shortcuts for timed events (e.g. `Alt + Up/Down` to move, `Alt + Shift + Up/Down` to resize) were skipped by design in favor of focusing on direct pointer manipulation, magnetic snapping, and spatial navigation polish. Deferred to post-release accessibility roadmap.

---

## Phase 5 — Month / Week / Day Spatial Continuity Transitions [COMPLETE]

Implemented spatial continuity transitions between Month, Week, and Day views, replacing abrupt view swaps with smooth, spatially anchored zoom animations.

### Capabilities:

- **Dynamic Spatial Anchor (`transform-origin`)**:
  - Automatically calculates precise `{ x, y }` percentage coordinates based on where `selectedDateKey` is positioned in the calendar grid.
  - **Month Transitions**: Both horizontal column (0..6, `x = ((col + 0.5) / 7) * 100%`) and vertical row (0..5, `y = ((row + 0.5) / 6) * 100%`) are anchored. When switching Month $\rightarrow$ Week or Month $\rightarrow$ Day, the view zooms out from the exact clicked/selected date cell.
  - **Week $\leftrightarrow$ Day Transitions**: `x` is anchored to the selected day's column in the week (`x = ((col + 0.5) / 7) * 100%`) and `y` is centered (50%). Focusing into Day zooms from that column position, and pulling back to Week pulls back toward that column.
  - **Date Targeting**: `selectMonthDate(dateKey)` explicitly passes the double-clicked target date so the spatial anchor reflects the newly selected date immediately.
  - **Authoritative Week Start & Single-Day Geometry Safety**: Uses `result.weekStartsOn` from `useCalendarWindow`. When transitioning from Day view (where `dateKeys` contains only 1 date), grid geometry bypasses single-day arrays and calculates the weekday column and month row relative to `weekStartsOn`.
- **Hierarchical Direction Mapping**:
  - Deterministic hierarchy: Day (0) < Week (1) < Month (2).
  - Deepening transitions (Month $\rightarrow$ Week, Month $\rightarrow$ Day, Week $\rightarrow$ Day) trigger `'in'` transitions (`viewTransitionZoomIn`).
  - Broadening transitions (Day $\rightarrow$ Week, Day $\rightarrow$ Month, Week $\rightarrow$ Month) trigger `'out'` transitions (`viewTransitionZoomOut`).
  - Same-mode re-selections produce `null` (no spurious transition).
- **Subtle Micro-Scale Keyframes & Easing**:
  - `viewZoomIn`: `scale(1.018)` to `transform: none` over 220ms with `cubic-bezier(0.16, 1, 0.3, 1)`.
  - `viewZoomOut`: `scale(0.982)` to `transform: none` over 220ms with `cubic-bezier(0.16, 1, 0.3, 1)`.
- **Chromium Text-Blur Hardening & Zero Lingering Styles**:
  - Exactly 240ms after initiating a transition, React state clears `transitionState` to `null`, completely removing the `transformOrigin` inline style and animation classes.
  - Large calendar container terminates strictly at `transform: none`.
  - Zero permanent `will-change: transform`, zero `animation-fill-mode: forwards`, and zero lingering fractional scaling, guaranteeing 100% crisp Chromium text rasterization.
- **Rapid Switching & State Machine Safety**:
  - Rapid clicking across views (e.g. Month $\rightarrow$ Week $\rightarrow$ Day) cancels any pending timer via `transitionTimerRef`, initiating the new transition smoothly without getting stuck in half-animated states.
  - Unmount cleanup ensures timers never fire on unmounted components.
- **Accessibility & Reduced Motion**:
  - React level: Bypasses transition state when `window.matchMedia('(prefers-reduced-motion: reduce)')` is active.
  - CSS level: `@media (prefers-reduced-motion: reduce)` overrides `.calendarViewTransition`, `.viewTransitionZoomIn`, `.viewTransitionZoomOut`, and `.viewTransitionFade` with `animation: none !important; transform: none !important; transition: none !important;`.
...
- [x] Phase 3.2 — origin ghost outline
- [x] Phase 3.3 — release settle animation (`8abbd0b`)
- [x] Phase 3.4 — timeline edge auto-scroll
- [x] Phase 3.5 — smooth motion & undo polish
- [x] Phase 4 — keyboard manipulation (SKIPPED / deferred by design)
- [x] Phase 5 — Month / Week / Day spatial continuity transitions
