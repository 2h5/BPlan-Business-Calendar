...
- **Tests / Verification:**
  ...
  - Deno lint/check passed for changed functions; `deno task test` passed with 147 tests. Local Supabase reset, `supabase test db --local` passed all 113 database tests, `pnpm db:types` completed with only the intended `return_target` generated-type additions, and `git diff --check` passed.
